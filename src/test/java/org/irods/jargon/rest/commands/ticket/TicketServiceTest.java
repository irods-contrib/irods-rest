...
public class TicketServiceTest implements ApplicationContextAware {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		ticketService = ticketServiceFactory
		...
		restrictionTypeValueMap.put("add_group", testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_GROUP_KEY));
		restrictionTypeValueMap.put("remove_group", testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_GROUP_KEY));
		restrictionTypeValueMap.put("add_user", testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_KEY));
		restrictionTypeValueMap.put("remove_user", testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_KEY));
	...
	public void setUp() throws Exception {
		...
		SpringResourceFactory noDefaults = new SpringResourceFactory(
				"ticketService", applicationContext, TicketService.class);
	...
	public void testCreateReadTicketDefaultStringXml() throws Exception {
		...
		String expectedResponse_1 = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
				...
				+ "<ticket_string>";
		String expectedResponse_2 = "</ticket_string>" + "</ns2:ticket>";
		...
		try {
			...
			Assert.assertTrue("Did not get expected xml stuff.  Sent: "
					+ requestBody + " Received: " + entityData + "Expected: "
					+ expectedResponse_1, index1 > -1);

			Assert.assertTrue("Did not get expected xml stuff.  Sent: "
					+ requestBody + " Received: " + entityData + "Expected: "
					+ expectedResponse_2, index2 > -1);

			int ticketLength = index2 - index1 - expectedResponse_1.length();

			Assert.assertTrue("Ticket length was " + ticketLength
					+ " Expected: 15", ticketLength == 15);

			ticketString = entityData.substring(
					index1 + expectedResponse_1.length(), index1
							+ expectedResponse_1.length() + 15);
			System.out.println("ticketId=" + ticketString);

			// check that the ticket actually exists
			Ticket t = ticketService
					.getTicketForSpecifiedTicketString(ticketString);

			// check that the ticket is read only
			Assert.assertTrue(
					"Ticket type was not correct.  Expected read but detected "
							+ t.getType(),
	...
	public void testCreateWriteTicketDefaultStringJson() throws Exception {
		...
		try {
			...
			Assert.assertTrue("Did not get expected xml stuff.  Sent: "
					+ requestBodyXml + " Received: " + entityData
	...
	public void createTicketWithSpecifiedStringJson() throws Exception {
		...
		String requestBody = "{\"mode\":\"read\",\"object_path\":\""
				+ targetIrodsFile + "\"," + "\"ticket_string\":\""
				+ ticketString + "\"}";

		System.out.println("ticket request json:" + requestBody);
	...
	public void deleteTicket() throws Exception {
		...
		IRODSAccessObjectFactory accessObjectFactory = irodsFileSystem
				.getIRODSAccessObjectFactory();
		IRODSFile file = accessObjectFactory.getIRODSFileFactory(irodsAccount)
				.instanceIRODSFile(targetIrodsFile);
		ticketService.createTicket(TicketCreateModeEnum.READ, file,
				ticketString);
		...
		try {
			...
			// test that the ticket has been deleted - should throw
	...
	public void updateTicketXml() throws Exception {
		...
		try {
			...
			for (String key : restrictionTypeValueMap.keySet()) {
				...
				clientAndContext = RestAuthUtils.httpClientSetup(irodsAccount,
						testingProperties);
				...
				HttpResponse response = clientAndContext.getHttpClient()
						.execute(httpput, clientAndContext.getHttpContext());
				Assert.assertEquals(204, response.getStatusLine()
						.getStatusCode());
				...
				case "add_host":
					tempList = ticketService
							.listAllHostRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Add host expected size of 1 but got size of "
									+ tempList.size(), tempList.size() == 1);
					break;
				case "remove_host":
					tempList = ticketService
							.listAllHostRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Remove host expected size of 0 but got size of "
									+ tempList.size(), tempList.size() == 0);
					break;
				case "add_group":
					tempList = ticketService
							.listAllGroupRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Add group expected size of 1 but got size of "
									+ tempList.size(), tempList.size() == 1);
					Assert.assertTrue(
							"Did not get the expected group for ticket.  "
									+ tempList.get(0),
				...
				case "remove_group":
					tempList = ticketService
							.listAllGroupRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Remove group expected size of 0 but got size of "
									+ tempList.size(), tempList.size() == 0);
					break;
				case "add_user":
					tempList = ticketService
							.listAllUserRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Add user expected size of 1 but got size of "
									+ tempList.size(), tempList.size() == 1);
					Assert.assertTrue(
							"Did not get the expected user for ticket.  "
									+ tempList.get(0),
							restrictionValue.equalsIgnoreCase(tempList.get(0)));
					break;
				case "remove_user":
					tempList = ticketService
							.listAllUserRestrictionsForSpecifiedTicket(
									t.getTicketString(), 0);
					Assert.assertTrue(
							"Remove user expected size of 0 but got size of "
									+ tempList.size(), tempList.size() == 0);
					break;
				case "byte_write_limit":
					Assert.assertTrue(
							"Did not get the expected write byte limit for ticket.  "
									+ t.getWriteByteLimit(), t
									.getWriteByteLimit() == Long
									.parseLong(restrictionValue));
					break;
				case "file_write_limit":
					Assert.assertTrue(
							"Did not get the expected write file limit for ticket.  "
									+ t.getWriteFileLimit(), t
									.getWriteFileLimit() == Long
									.parseLong(restrictionValue));
					break;
				case "uses_limit":
					Assert.assertTrue(
							"Did not get the expected uses limit for ticket.  "
									+ t.getUsesLimit(),
							t.getUsesLimit() == Long
									.parseLong(restrictionValue));
					break;
				case "expiration":
					Assert.assertTrue(
							"Did not get the expected expiration data.",
							t.getExpireTime().equals(
			...
			if (clientAndContext != null)
				clientAndContext.getHttpClient().getConnectionManager()
						.shutdown();
	...
	public void updateTicketJson() throws Exception {
		...
		try {
			...
			for (String key : restrictionTypeValueMap.keySet()) {
				...
				String requestBody = "{\"restriction_type\":\""
						+ restrictionType + "\"," + "\"restriction_value\":\""
						+ restrictionValue + "\"}";
	...
	public void listTicketXml() throws Exception {
		...
		String ticketId = ticketService.getTicketForSpecifiedTicketString(
				ticketString).getTicketId();

		ticketService.addTicketHostRestriction(ticketString, "localhost");
		ticketService.addTicketUserRestriction(ticketString, testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_KEY));
		ticketService.addTicketGroupRestriction(ticketString, testingProperties
				.getProperty(TestingPropertiesHelper.IRODS_USER_GROUP_KEY));
		...
		Date expirationTime = new Date(new Date().getTime() + 365 * 24 * 60
				* 60 * 1000L);
		...
		String expectedResponse1 = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
				...
				+ "<expire_time>"
				+ EXPIRATION_DATE_FORMAT.format(expirationTime)
				+ "</expire_time>"
				+ "<irods_path>"
				+ targetIrodsFile
				+ "</irods_path>" + "<host_restrictions>";
		...
		try {
			...
			// The host can be translated to IP so just look for the part before
			// and after host restrictions.
			int index1 = entityData.indexOf(expectedResponse1);
			int index2 = entityData.indexOf(expectedResponse1);

			Assert.assertTrue("Did not get expected xml stuff.  Received: "
					+ entityData + "Expected: " + expectedResponse1,
					index1 > -1);
			Assert.assertTrue("Did not get expected xml stuff.  Received: "
					+ entityData + "Expected: " + expectedResponse2,
					index2 > -1);
	...
	public void listTicketJson() throws Exception {
		...
		String expectedResponse1 = "{\"ticket_id\":\""
				+ ticketId
				+ "\","
				+ "\"ticket_string\":\""
				+ ticketString
				...
				+ testingProperties
						.getProperty(TestingPropertiesHelper.IRODS_ZONE_KEY)
				+ "\"," + "\"uses_count\":0," + "\"uses_limit\":20,"
				+ "\"write_file_count\":0," + "\"write_file_limit\":10,"
				+ "\"write_byte_count\":0," + "\"write_byte_limit\":1000,"
				+ "\"expire_time\":\""
				+ EXPIRATION_DATE_FORMAT.format(expirationTime) + "\","
