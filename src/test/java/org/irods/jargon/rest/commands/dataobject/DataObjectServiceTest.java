...
public class DataObjectServiceTest implements ApplicationContextAware {
	...
	public void setUp() throws Exception {
		...
		SpringResourceFactory noDefaults = new SpringResourceFactory("dataObjectService", applicationContext,
	...
	public void testGetDataObjectData() throws Exception {
		...
		try {
			...
			DataObjectData actual = objectMapper.readValue(entityData, DataObjectData.class);
			...
			Assert.assertEquals(actual.getCollectionId(), dataObject.getCollectionId());
			Assert.assertEquals(actual.getCollectionName(), dataObject.getCollectionName());
			Assert.assertEquals(actual.getComments(), dataObject.getComments());
			Assert.assertEquals(actual.getDataMapId(), dataObject.getDataMapId());
			Assert.assertEquals(actual.getDataName(), dataObject.getDataName());
			Assert.assertEquals(actual.getDataOwnerName(), dataObject.getDataOwnerName());
			Assert.assertEquals(actual.getDataOwnerZone(), dataObject.getDataOwnerZone());
			Assert.assertEquals(actual.getDataPath(), dataObject.getDataPath());
			Assert.assertEquals(actual.getDataReplicationNumber(), dataObject.getDataReplicationNumber());
			Assert.assertEquals(actual.getDataSize(), dataObject.getDataSize());
			Assert.assertEquals(actual.getDataStatus(), dataObject.getDataStatus());
			Assert.assertEquals(actual.getDataTypeName(), dataObject.getDataTypeName());
			Assert.assertEquals(actual.getDataVersion(), dataObject.getDataMapId());
			Assert.assertEquals(actual.getExpiry(), dataObject.getExpiry());
			Assert.assertEquals(actual.getId(), dataObject.getId());
			Assert.assertEquals(actual.getObjectPath(), dataObject.getObjectPath());
			Assert.assertEquals(actual.getReplicationStatus(), dataObject.getReplicationStatus());
			Assert.assertEquals(actual.getResourceGroupName(), dataObject.getResourceGroupName());
			Assert.assertEquals(actual.getResourceName(), dataObject.getResourceName());
			Assert.assertEquals(actual.getCreatedAt(), dataObject.getCreatedAt());
			Assert.assertEquals(actual.getSpecColType(), dataObject.getSpecColType());
			Assert.assertEquals(actual.getUpdatedAt(), dataObject.getUpdatedAt());
	...
	public void testDeleteDataObjectData() throws Exception {
		...
		try {
			...
			IRODSFile actual = accessObjectFactory.getIRODSFileFactory(irodsAccount).instanceIRODSFile(targetIrodsFile);
	...
	public void testGetMetadataListingJson() throws Exception {
		...
		String absPath = scratchFileUtils.createAndReturnAbsoluteScratchPath(IRODS_TEST_SUBDIR_PATH);
		String localFileName = FileGenerator.generateFileOfFixedLengthGivenName(absPath, testLocalName, 1);
		...
		DataTransferOperations dto = accessObjectFactory.getDataTransferOperations(irodsAccount);
		dto.putOperation(localFileName, targetIrodsCollection + "/" + testFileName,
				testingProperties.getProperty(TestingPropertiesHelper.IRODS_RESOURCE_KEY), null, null);
	...
	public void testBulkAddDataObjectAVUSendJson() throws Exception {
		...
		try {
			...
			DataObjectAO dataObjectAO = accessObjectFactory.getDataObjectAO(irodsAccount);
			List<MetaDataAndDomainData> actualList = dataObjectAO.findMetadataValuesForDataObject(dataObjectAbsPath);
	...
	public void testAddPermission() throws Exception {
		...
		try {
			...
			UserFilePermission actualFilePermission = dataObjectAO
					.getPermissionForDataObjectForUserName(targetIrodsFile, secondaryAccount.getUserName());
	...
	public void testDeletePermission() throws Exception {
		...
		sb.append("/acl/");
		sb.append(secondaryAccount.getUserName() + "," + secondaryAccount.getZone());
		...
		try {
			...
			UserFilePermission actualFilePermission = dataObjectAO
					.getPermissionForDataObjectForUserName(targetIrodsFile, secondaryAccount.getUserName());
			Assert.assertTrue("permission not deleted", actualFilePermission == null);
